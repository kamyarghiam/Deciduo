...
class PygameGame(object):
	...
	def init(self, mode = "Startscreen", profile = None,LeftWord = "Log in",RightWord = "New User", information = []):
		...
		if self.mode == "Startscreen":
			...
			self.versionSurface = (PygameGame.smallerfont).render('Version 6.1 By Kamyar Ghiam', False, (255,255,255))
		...
		if self.mode == "Startscreen":
			...
			try: 
				if self.exerciseResultsSubmit.isCollide():
					...
					self.mainMenuButton = Button(170, 50, self.width//2 +200,self.height//2 +100, self.buttonColor, "Main Menu",20, 7)
				...
				else: self.menuButtonActive = False
			except: pass
	...
	def mouseMotion(self, x, y):
		self.mousex = x
		self.mousey = y
		#checks if back button is pressed and changes buttons accordingly
		def checkBack():
			try:
				if self.back.isCollide():
	...
	def redrawAll(self, screen):
		...
		if self.mode == "Startscreen":
			...
			self.restart = Button(200, 50, self.width-150,self.height-150,self.restartButtonColor, "Restart Search",12, 7)
			...
			if exercisenumbers >= 3:
				...
				self.exerciseResultsbutton = Button(170, 50, 775,70, self.greenFont, "Final Step",20, 7)
