...
class PygameGame(object):
	...
	def init(self, mode = "Startscreen", profile = None,LeftWord = "Log in",RightWord = "New User"):
		...
		if self.mode == "Startscreen":
			...
			self.versionSurface = (PygameGame.smallerfont).render('Version 5.0 By Kamyar Ghiam', False, (255,255,255))
		...
		if self.mode == "Startscreen":
			...
			try:
				if self.exerciseResultsSubmit.isCollide():
					...
					self.mainMenuButton = Button(100, 50, 170,200, self.buttonColor, "Main Menu",20, 7)
	...
	def redrawAll(self, screen):
		...
		if self.mode == "Startscreen":
			...
			ask = Text(50, self.height//2 +50, "How much time would you like to spend? (Must be larger than %0.1f to meet calorie requirement)" % minimum_time, 20)
			ask.write(screen)
			...
			results = Text(100,100, self.exerciseResultsText, 20)
			results.write(screen)
			try:
				self.mainMenuButton.drawBox(screen)
